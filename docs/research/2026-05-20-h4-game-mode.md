...
### Update (2026-05-21): fly mode + content-aimed spawn

First playtest on `valley` surfaced the deepest UX problem: the player
spawned *inside* the splat soup looking out, and the scene appeared
upside-down. Root cause — arbitrary 3DGS captures share no common
up-axis or ground plane, so gravity-walk drops the player in an
unpredictable direction, and a bbox-centre spawn lands inside the dense
geometry.

The fix is **fly (creative) mode as the game-mode default**:

- No gravity; `Space` ascends, `Shift` descends, WASD follows the
  camera's full 3D forward (look up + W flies up-and-forward). `F`
  toggles walk mode for scenes that happen to be Y-up.
- Collision is still enforced — you stop at solid cells, so you can
  land on a surface and carve it from outside.
- The player is now sized as a fraction of the **bbox diagonal**
  (the `playerSizeFraction` scene-config field, which was added but
  never wired until now) rather than voxel size.
- Spawn is a diagonal step *outside* the densest occupied voxel,
  camera aimed at that voxel — a 3/4 establishing view of the actual
  content, guaranteed open air so the player never starts wedged
  inside geometry.

![valley fly-mode spawn](images/2026-05-21-game/valley-fly-spawn.png)

`?mode=game&scene=valley` now opens looking at the terrain ridge,
right-side-up, in fly mode. This is the honest answer to "voxel game on
an arbitrary splat scene": a 6-DOF creative-fly editor, because
gravity-walk needs a known up-axis the capture doesn't provide.
